...
public class BattleBot extends Thread {
	...
	public void run()
	{
		BufferedImage mapPic = R.createScreenCapture(new Rectangle((int)screenWidth - 408, ((int)screenHeight - 407), 400, 398));
		//ImageIO.write(mapPic, "png", new File("mapPic.png"));
		
		
		
		//Testing an individual pixel for map identification
		int mapID = new Color(mapPic.getRGB(399, 0)).getRed();
		if(mapID == 52)
		{
			currentMap = "Malinovka";
			malinovka();
		}
		else if(mapID == 97)
		{
		...
		{
			currentMap = "Winter Himmelsdorf";
			winterHimmelsdorf();
		}
		else if(mapID == 28)
		{
			currentMap = "Himmelsdorf";
			himmelsdorf();
		}
		else
		{
			currentMap = "Unknown Map!";
		}
		...
		for(int i = 0; i < 218; i++)
		{
			if(new Color(hpPic.getRGB(i, 0)).getGreen() > 20 || 
			   new Color(hpPic.getRGB(i, 0)).getRed() > 20 ||
			   new Color(hpPic.getRGB(i, 0)).getBlue() > 20)
			{
				hpCount++;
			}
			else
		...
		double hpPercent = (hpCount/218) * 100;
		*/
		
		while(true)
		{
			System.out.println("HELLO FROM BATTLEBOT");
		boolean found = false;
		int numAllies = 0;
		int numEnemies = 0;
		
		allyLocations = new Point[15];
		enemyLocations = new Point[15];
		
			for(int x = 0; x < mapPic.getWidth(); x++)	//i == x;
			{
				for(int y = 0; y < mapPic.getHeight(); y++)
				{
					Color c = new Color(mapPic.getRGB(x, y));
					
					if(c.getRed() < 30 && c.getGreen() > 180 && c.getBlue() > 180)
					{
						//System.out.println("R: " + c.getRed() + ", G: " + c.getGreen() + ", B:" + c.getBlue());
						//System.out.println("x:" + x + " y:" + y);
						if(numAllies < 15)
						{
							allyLocations[numAllies] = new Point(x, y);
							numAllies++;
							globalMap[x][y] = 2;
						}
					}
					
					if(c.getRed() > 205 && c.getGreen() < 20 && c.getBlue() < 20)
					{
						//System.out.println("R: " + c.getRed() + ", G: " + c.getGreen() + ", B:" + c.getBlue());
						//System.out.println("x:" + x + " y:" + y);
						enemyLocations[numEnemies] = new Point(x, y);
						numEnemies++;
						globalMap[x][y] = 3;
					}
					
					if(c.getRed() > 150 && c.getGreen() > 150 && c.getBlue() < 50)
					{
						found = true;
						globalMap[x][y] = 1;
					}
		...
		//System.out.println("# Allies:" + numA +", # Enemies:" + numE);
		//System.out.println(hpPercent);
			for(Point p: allyLocations)
			{
				if(p != null)
				{
